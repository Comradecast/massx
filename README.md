...
- Multi-victim cases with deterministic public-location wording such as `street`, `sidewalk`, `intersection`, `block`, `outside`, `parking lot`, `gas station`, `in the area`, or `nearby` are promoted into `public_space_nonrandom` when no stronger domestic, party, school, nightlife, or event context applies.
