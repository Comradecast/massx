...
def main() -> None:
    ...
    labels = [incident_label(row) for _, row in filtered_queue.iterrows()]
    selected_label = st.sidebar.selectbox("Select incident", labels)
    selected_index = labels.index(selected_label)
    selected_row = filtered_queue.iloc[selected_index]
    incident_id = str(selected_row["incident_id"])
    existing_result = get_existing_result(results_frame, incident_id)

    st.subheader(f"Incident {incident_id}")
    status_text = "Saved review exists" if existing_result["incident_id"] else "No saved review yet"
    ...
    with details_col_2:
        ...
        st.write(f"Selected source URL: {selected_row.get('selected_source_url', '')}")
